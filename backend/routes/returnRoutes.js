...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    // Update order status to pending return/replacement
    ...
    // Notify seller (notifications still use User model for now)
    if (seller) {
...
// Debug endpoint to list all returns
...
router.get('/seller', authMiddleware, async (req, res) => {
  try {
    ...
    // If user is a seller, find returns where seller matches their ID
    // If user is admin, they should see all returns (but this endpoint is for sellers)
