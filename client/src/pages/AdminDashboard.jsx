  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [statsRes, productsRes, ordersRes] = await Promise.all([
        api.get('/api/users/stats', { headers: { Authorization: `Bearer ${token}` } }),
        api.get('/api/products', { headers: { Authorization: `Bearer ${token}` } }),
        api.get('/api/orders/all', { headers: { Authorization: `Bearer ${token}` } }),
      ]);
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 401 || err.response?.status === 403) {
        errorMessage = 'Unauthorized: Please ensure you are logged in as an admin';
    ...
    <div className="container min-h-screen bg-gray-50 dark:bg-gray-900 py-6 px-4 sm:px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        {[
          ...
          { title: 'Total Orders', value: stats.totalOrders || 0, icon: <Package size={20} /> },
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-8">
        ...
        <form onSubmit={handleAddProduct} className="space-y-4">
          ...
          <div>
            ...
            />
            <p id="image-instructions" className="mt-1 text-sm text-gray-500 dark:text-gray-400">
